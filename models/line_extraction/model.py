...
class ColumnDetectionModel(nn.Module):
    ...
    def forward(self, images: torch.Tensor, targets: Dict[str, torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        if self.training and targets is not None:
            # 学習時
            loss_dict = self.model(images, targets, augment=True)
            return loss_dict
        else:
            # 推論時
            predictions = self.model(images, augment=False, conf=self.conf_threshold, iou=self.nms_threshold)
    ...
    def predict(self, image: torch.Tensor, score_threshold: float = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        predictions = self.model(image, augment=False, conf=threshold, iou=self.nms_threshold)

        if len(predictions[0]) == 0:
            # 検出なしの場合
            return (torch.zeros((0, 4), device=image.device), torch.zeros(0, device=image.device))

        return (predictions[0].boxes.xyxy, predictions[0].boxes.conf)
