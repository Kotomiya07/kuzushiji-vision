...
class QuadrantProcessor:
    ...
    def split_image(self, image_path: str) -> dict[str, np.ndarray]:
        """
        画像を4分割する
        ...
        for position, (x1, y1, x2, y2) in self.quadrants.items():
            # 正規化座標をピクセル座標に変換
    ...
    def convert_annotations(self, annotation_path: str, quadrant_position: str) -> list[str]:
        """
        アノテーションを分割後の画像に対応する座標に変換する

        Args:
            annotation_path: 元のアノテーションファイルのパス
            quadrant_position: 分割位置 (top_left, top_right, bottom_left, bottom_right)
        ...
        # 分割エリアの正規化座標
        area_x1, area_y1, area_x2, area_y2 = self.quadrants[quadrant_position]
    ...
    def process_single_image(
        ...
        """
        単一の画像とアノテーションを元画像+4分割（計5枚）で処理する
        ...
        Returns:
            Dict[str, int]: 各位置（original + 4分割）でのアノテーション数
        """
        # 出力ディレクトリの作成
        ...
        # 2. 画像を4分割
        split_images = self.split_image(image_path)

        for position, image_array in split_images.items():
            # 分割画像の保存
            ...
            # アノテーション変換
            converted_annotations = self.convert_annotations(annotation_path, position)
            annotation_counts[position] = len(converted_annotations)
