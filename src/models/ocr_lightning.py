...
# Assuming Vocab is imported where LitOCRModel is used, and PAD_ID, GO_ID, EOS_ID from Vocab attributes
from ..utils.tokenizer import Vocab # Vocab class itself
from .unet_transformer_encoder import UNetTransformerEncoder
from ..utils.metrics import compute_iou


class LitOCRModel(pl.LightningModule):
    def __init__(self, model_config, optimizer_config, tokenizer: Vocab): # Added tokenizer argument
        super().__init__()
        ...
        # Loss functions
        self.criterion_char = nn.CrossEntropyLoss(ignore_index=self.vocab.pad_id) # Use pad_id from tokenizer
    ...
    def forward(self, images, target_ids=None):
        ...
        if target_ids is not None:
            ...
            dummy_input_ids = torch.full((images.size(0), 1), self.vocab.go_id, dtype=torch.long, device=self.device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_mask = (target_char_ids != PAD_ID) # Shape: (B, max_label_len-1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        decoder_input_ids = target_ids_full[:, :-1] # From <go> to token before <eos>
        target_char_ids = target_ids_full[:, 1:]    # From first char to <eos>
        loss_mask = (target_char_ids != self.vocab.pad_id)     # Shape: (B, max_label_len-1)
        ...
        generated_ids = self.decoder.generate(
            input_ids=torch.full((images.size(0), 1), self.vocab.go_id, dtype=torch.long, device=self.device),
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss_mask = (target_char_ids != self.vocab.pad_id)
