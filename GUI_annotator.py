...
class DataManager:
    ...
    def add_column(self, original_image_path, new_box_in_original):
        ...
        try:
            ...
            # 既存のファイル名と衝突しないようにする (例: newcol_01.jpg)
            existing_files = list(save_dir.glob("newcol*.jpg"))
            # 数字以外が含まれている場合は無視
            existing_files = [f for f in existing_files if f.stem.split("_")[-1].isdigit()]
            # 既存のファイルがあれば、最大の数字を取得して+1する
            if existing_files:
                existing_numbers = [int(f.stem.split("_")[-1]) for f in existing_files if "_" in f.stem]
    ...
    def _recreate_column_from_chars(self, chars_in_orig, u_ids, original_img_path, base_rel_path_str, suffix):
        ...
        try:
            ...
            new_filename = f"{base_path.stem}{suffix}.jpg"
            # 保存先ディレクトリを元のパスから取得
...
class AnnotatorApp:
    ...
    def load_current_image_data(self):
        ...
        if not columns_df.empty:
            ...
            colors = ["orange", "green", "cyan", "blue", "purple", "magenta"]
