...
import numpy as np
from skimage.transform import resize # Pillowだけだと高品質なリサイズが少し面倒なため
import shutil
import os
...
class ImageCanvas(tk.Canvas):
    ...
    def load_image(self, image_path):
        try:
            ...
            self.original_pil_image = Image.open(self.image_path).convert("RGBA") # RGBAで透明度対応
            self.scale = 1.0
            self.boxes = []
            self.selected_box_tags = set()
            self.box_id_map = {}
    ...
    def display_image(self):
        ...
        try:
            # numpy経由でリサイズし、Pillow Imageに戻す
            img_array = np.array(self.original_pil_image)
            # RGBAの場合、チャネル数は4
            resized_array = resize(img_array, (height, width), anti_aliasing=True, preserve_range=True).astype(img_array.dtype)
            self.pil_image = Image.fromarray(resized_array, mode='RGBA' if img_array.shape[2] == 4 else 'RGB')
    ...
    def redraw_boxes(self):
        ...
        if not self.pil_image: return # 画像がない場合は描画しない
        ...
        for box_info in self.boxes:
            ...
            if box_info["text"]:
                ...
                try:
                    font = ImageFont.truetype("arial.ttf", font_size) # 環境に合わせてフォント指定
                except IOError:
                    font = ImageFont.load_default() # フォントが見つからない場合

                self.create_text(x1 + 2, y1 + 2, text=box_info["text"], anchor="nw", fill=color, tags=("box_text", tag), font=font)
...
class DataManager:
    ...
    def load_data(self):
        ...
        if 'original_image' in self.df.columns:
            ...
            self.original_images.sort() # ソートしておく
    ...
    def get_original_image_abs_path(self, original_image_path_str):
         """ 元画像の絶対パスを取得 (rawデータからの相対パスと仮定) """
         # data_preprocessing.py の出力を考慮すると、`original_image` は
         # `data/raw/dataset/doc_id/images/img_name.jpg` のような形式のはず。
         # しかし、csvには絶対パスで保存されるか、実行環境からの相対パスの場合もある。
         # ここでは、csvの値が `data_dir` からの相対パスだと仮定して構築する。
         # もし絶対パスなら Path(original_image_path_str)だけで良い。
         # ここでは、より汎用的に `base_path` の親を基準とする。
         base_data_dir = self.base_path.parent # data/ を想定
         # パスの構造を仮定 (raw/dataset/...)
         # 注意: このパス解決は環境やcsvの内容に依存するため、要調整
         potential_path = Path(original_image_path_str)
         if potential_path.is_absolute():
             return potential_path
         else:
             # base_path (例: data) からの相対パスと解釈してみる
             abs_path = self.base_path / original_image_path_str
             if abs_path.exists():
                 return abs_path
             # あるいは、プロジェクトルートからの相対パスかもしれない
             # ここでは data/raw/dataset 構造を決め打ちで試す
             # 'dataset' の部分は設定ファイルから読むべきかもしれない
             raw_path_part = Path(*Path(original_image_path_str).parts[-4:]) # 例: dataset/00001/images/00001_001.jpg
             abs_path = base_data_dir / "raw" / raw_path_part
             if abs_path.exists():
                  return abs_path
             else:
                 # 見つからない場合は元の値を返す（エラー表示は呼び出し元で）
                 print(f"Warning: Could not determine absolute path for original image: {original_image_path_str}")
                 return potential_path
    ...
    def split_column(self, column_path_to_split, split_index):
        ...
        def create_new_column_from_chars(chars_in_orig, u_ids, orig_img_path, base_rel_path_str, suffix):
            if not chars_in_orig: return None, None
            ...
            if crop_x1 >= crop_x2 or crop_y1 >= crop_y2: return None, None
        ...
        if new_data1 is None or new_data2 is None:
            ...
            if new_path1: os.remove(self.processed_dir / new_path1)
            if new_path2: os.remove(self.processed_dir / new_path2)
            return False
    ...
    def move_characters(self, src_column_path, target_column_path, char_indices_to_move):
        ...
        new_src_data, new_src_path = recreate_column(remaining_src_chars_orig, remaining_src_ids, src_data['original_image'], src_column_path, "_src")

        # 移動先の新しい列データ
        new_tgt_data, new_tgt_path = recreate_column(final_tgt_chars_orig, final_tgt_ids, tgt_data['original_image'], target_column_path, "_tgt")

        if new_tgt_data is None: # 移動先が (理論上ありえないが) 不正
             messagebox.showerror("エラー", "移動先の列の再生成に失敗しました。")
             # 作成途中のファイルを削除
             if new_src_path: os.remove(self.processed_dir / new_src_path)
             return False

        # --- DataFrame更新 ---
        indices_to_drop = self.df[(self.df['column_image'] == src_column_path) | (self.df['column_image'] == target_column_path)].index
        self.df = self.df.drop(indices_to_drop)
        ...
        except Exception as e: print(f"Warning: could not delete {src_column_path}: {e}")
        try:
            os.remove(self.get_column_abs_path(target_column_path))
            print(f"Deleted old column image: {target_column_path}")
        except Exception as e: print(f"Warning: could not delete {target_column_path}: {e}")
    ...
    def delete_character(self, column_path, char_index_to_delete):
         ...
         new_col_data, new_col_path = recreate_column_after_delete(new_char_boxes_orig, new_unicode_ids, original_image_path, column_path, "")
...
class AnnotatorApp:
    ...
    def load_current_image_data(self):
        ...
        if orig_abs_path and orig_abs_path.exists():
            self.orig_canvas.load_image(orig_abs_path)
             ...
             self.orig_canvas.clear_canvas()
             # エラーメッセージ表示
             self.orig_canvas.create_text(self.orig_canvas.winfo_width() // 2, 20, text=f"元画像が見つかりません:\n{orig_abs_path}", fill="red", anchor="n")
        ...
        if not columns_df.empty:
            ...
            columns_df['sort_key'] = columns_df['box_in_original'].apply(lambda x: x[0] if isinstance(x, list) and len(x)>0 else float('inf'))
            ...
            for i, (_, row) in enumerate(columns_df.iterrows()):
                ...
                # 元画像にボックス描画
                if 'box_in_original' in row and isinstance(row['box_in_original'], list) and len(row['box_in_original']) == 4:
                    x1, y1, x2, y2 = row['box_in_original']
                    color = colors[i % len(colors)]
                    # タグとして列の相対パスを使用 (ユニークにするため)
                    self.orig_canvas.add_box(tag=col_rel_path, x1=x1, y1=y1, x2=x2, y2=y2, color=color, width=2, text=f"Col {i+1}")
                else:
                    print(f"Warning: Invalid 'box_in_original' for {col_rel_path}")
    ...
    def on_column_select(self, event=None):
        ...
        if not selected_indices:
            ...
            all_columns_df['sort_key'] = all_columns_df['box_in_original'].apply(lambda x: x[0] if isinstance(x, list) and len(x)>0 else float('inf'))
    ...
    def load_detail_column(self, column_rel_path):
        ...
        if col_abs_path.exists():
                ...
                if isinstance(char_boxes, list) and isinstance(uids, list) and len(char_boxes) == len(uids):
                    for i, (box, uid) in enumerate(zip(char_boxes, uids)):
                        if isinstance(box, list) and len(box) == 4:
                            ...
                            self.detail_canvas.add_box(tag=char_tag, x1=x1, y1=y1, x2=x2, y2=y2, color="green", width=1, text=f"{i}:{uid}")
                        else:
            ...
            self.detail_canvas.create_text(self.detail_canvas.winfo_width() // 2, 20, text=f"列画像が見つかりません:\n{col_abs_path}", fill="red", anchor="n")


    def highlight_selected_columns_on_orig(self):
        """ 元画像上で選択中の列をハイライト """
        if not hasattr(self.orig_canvas, 'boxes'): return # 初期化中など
    ...
    def initiate_move_character(self):
        if not self.current_detail_column_path: return
        selected_char_tags = self.detail_canvas.get_selected_tags()
        if not selected_char_tags: return
        ...
        self.moving_characters_info = {
            "src_path": self.current_detail_column_path,
        ...
        if self.moving_characters_info and clicked_tags:
            ...
            if target_col_path:
                 ...
                 if src_path == target_col_path:
                      ...
                      # 確認ダイアログ
                      confirm = messagebox.askyesno("文字移動確認",
    ...
    def delete_selected_column(self):
        if not self.selected_column_paths: return
    ...
    def delete_selected_character(self):
        ...
        confirm = messagebox.askyesno("文字削除確認", f"{len(char_indices)}個の文字を列 '{Path(self.current_detail_column_path).name}' から削除しますか？")
        if confirm:
            deleted_count = 0
    ...
    # --- ナビゲーション ---
    def next_image(self):
        if not self.data_manager.original_images: return
    ...
    def check_unsaved_changes(self):
        ...
        if self.data_manager.changes_made:
            response = messagebox.askyesnocancel("未保存の変更", "未保存の変更があります。保存しますか？\n(「いいえ」で変更を破棄して続行、「キャンセル」で操作中断)")
...
if __name__ == "__main__":
    ...
    data_dir = filedialog.askdirectory(
        title="データディレクトリを選択してください (例: 'data' フォルダ)",
    ...
    if not data_dir:
        ...
        if not (data_path / "processed" / "column_info.csv").exists():
             messagebox.showwarning("確認", f"選択されたディレクトリに 'processed/column_info.csv' が見つかりません。\nパス: {data_path / 'processed'}\n\nツールは起動しますが、データは読み込めません。")
             # それでも起動は試みる
