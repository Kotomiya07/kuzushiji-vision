...
@torch.no_grad()
def validate(
    model: nn.Module, val_loader: DataLoader, device: torch.device, iou_threshold: float = 0.5
) -> Tuple[float, float]:
    """検証を実行
    ...
    accuracy, _ = compute_character_accuracy(
        pred_labels_list, gt_labels_list, pred_boxes_list, gt_boxes_list, iou_threshold=iou_threshold
    )
...
def main():
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=config["training"]["batch_size"], shuffle=True, num_workers=4, collate_fn=lambda x: x
    )
