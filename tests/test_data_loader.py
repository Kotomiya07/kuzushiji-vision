...
from scripts.data_loader import OCRDataset, build_char_to_int_map, get_data_loader
...
def temp_data_dir():
    ...
    for split, num_samples in num_samples_per_split.items():
        ...
        for i in range(num_samples):
            ...
            # Generate labels that ensure all characters are used across samples
            # For train split, create labels that collectively contain all chars
            if split == "train":
                if i == 0:
                    # First sample: use first part of alphabet + some numbers
                    label_text = "abcdefg1234567"  # 13 chars
                elif i == 1:
                    # Second sample: use middle part of alphabet + some numbers
                    label_text = "hijklmnop890"  # 11 chars
                else:
                    # Third sample: use remaining chars
                    label_text = "qrstuvwxyz0"  # 11 chars
            else:
                # For val/test, use varied length labels
...
def test_ocr_dataset(temp_data_dir, char_maps):
    ...
    if len(dataset) > 0:
        ...
        assert sample["bounding_boxes"].shape == (max_label_length, 4), (
            f"Bounding_boxes shape mismatch, expected ({max_label_length}, 4)"
        )
...
def test_get_data_loader(temp_data_dir, char_maps):
    ...
    from scripts.data_loader import DEFAULT_TARGET_HEIGHT, DEFAULT_TARGET_WIDTH
