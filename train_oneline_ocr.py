...
def setup_parser():
    ...
    parser.add_argument(
        "--data_root_dir",
        type=str,
        default="data/column_dataset_padded",
        help="Root directory containing train, val, test split folders (each with BookID subfolders).",
    )
    parser.add_argument(
        "--train_dir_name", type=str, default="train", help="Name of the training data folder under data_root_dir."
    )
    parser.add_argument(
        "--val_dir_name", type=str, default="val", help="Name of the validation data folder under data_root_dir."
    )
    parser.add_argument("--test_dir_name", type=str, default="test", help="Name of the test data folder under data_root_dir.")
    parser.add_argument(
        "--unicode_csv_path",
        type=str,
        default="data/unicode_translation.csv",
        help="Path to unicode_translation.csv for Vocab.",
    )
    ...
    parser.add_argument(
        "--image_channels", type=int, default=3, choices=[1, 3], help="Number of image channels (1 for grayscale, 3 for RGB)."
    )
    ...
    parser.add_argument(
        "--accelerator", type=str, default="gpu", choices=["cpu", "gpu", "tpu", "ipu", "auto"], help="Accelerator to use."
    )
    parser.add_argument(
        "--devices",
        type=str,
        default="1",
        help="Number of devices to use (e.g., 1 for single GPU, '0,1' for two GPUs, or number for multi-GPU).",
    )
    ...
    parser.add_argument(
        "--encoder_in_channels", type=int, default=3, help="Input channels for encoder (should match image_channels)."
    )
    parser.add_argument("--encoder_initial_filters", type=int, default=64, help="Initial filters in the U-Net encoder.")
    parser.add_argument(
        "--encoder_num_unet_layers", type=int, default=4, help="Number of U-Net downsampling layers in encoder."
    )
    parser.add_argument(
        "--encoder_num_transformer_layers", type=int, default=2, help="Number of Transformer layers in encoder."
    )
    parser.add_argument(
        "--encoder_transformer_heads", type=int, default=8, help="Number of attention heads in encoder's Transformer."
    )
    parser.add_argument(
        "--encoder_transformer_mlp_dim", type=int, default=2048, help="Dimension of the MLP in encoder's Transformer."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="val_cer",
        help="Metric to monitor for ModelCheckpoint (e.g., val_loss, val_cer, val_iou).",
    )
    parser.add_argument(
        "--checkpoint_mode",
        type=str,
        default="min",
        choices=["min", "max"],
        help="Mode for ModelCheckpoint ('min' for loss/error, 'max' for accuracy/iou).",
    )
...
def main(args):
    ...
    if args.wandb:
        logger = WandbLogger(
            ...
            name=f"{args.decoder_model_name.split('/')[-1]}-{datetime.now().strftime('%Y%m%d_%H%M%S')}",
