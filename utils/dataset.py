...
class CharacterDetectionDataset(Dataset):
    ...
    def __init__(self, column_image_dir: str, annotation_file: str, target_width: int = 192, transform=None):
        """
        Args:
    ...
    def __getitem__(self, idx: int) -> Dict[str, np.ndarray]:
        ...
        # データ拡張
        if self.transform:
            transformed = self.transform(image=resized_image, bboxes=boxes, labels=labels)
            resized_image = transformed["image"]
            boxes = np.array(transformed["bboxes"])
            labels = np.array(transformed["labels"])

        return {"image": resized_image, "boxes": boxes, "labels": labels, "scale": scale}
