...
def main():
    ...
    # 設定の読み込み
    with open("src/configs/model/yolo_oneline.yaml", encoding="utf-8") as f:
        config = yaml.safe_load(f)

    # 実験ディレクトリの設定
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    exp_dir = Path("experiments/yolo_oneline") / timestamp
    ...
    # 学習の設定
    train_args = {
        "data": "src/configs/data/yolo_oneline.yaml",
        ...
        "device": "auto",
        "workers": 24,
        "project": "experiments/yolo_oneline",
        "name": timestamp,
        "exist_ok": True,
        "pretrained": False,
