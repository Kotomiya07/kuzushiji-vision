...
def calculate_average_background_color_otsu(pil_image: Image.Image, original_image_path: str) -> tuple[int, int, int] | None:
    ...
    try:
        ...
        print(f"Error calculating average background color (Otsu) for {original_image_path}: {e}. Falling back to overall average.", file=sys.stderr)
        try:
...
def _process_single_image(image_path: str, target_width: int, max_height: int, input_dir: str, output_dir: str) -> tuple[str, bool, str | None]:
    """
    単一の画像を処理するワーカー関数。
...
def process_images_parallel(input_dir: str, output_dir: str, target_width: int):
    ...
    worker_func_with_args = partial(_process_single_image, 
