...
def analyze_image_sizes(root_dir_path: Path):
    ...
    if not image_files:
        print(f"エラー: パターン '{root_dir_path / target_pattern}' に一致するファイルが見つかりませんでした。", file=sys.stderr)
...
def plot_size_scatter(size_counter: Counter, save_path: str = None, size_scale_factor: float = 5.0):
    ...
    scatter = ax.scatter(widths, heights, s=scaled_marker_sizes, c=counts,
                         cmap='viridis', alpha=0.7, edgecolors='k', linewidth=0.5)

    # カラーバーを追加 (マーカーの色がどのカウント数に対応するかを示す凡例)
    cbar = fig.colorbar(scatter, label='カウント数')
...
if __name__ == "__main__":
    ...
    if not root_dir_path.is_dir():
         ...
         print(f"正しいパスを --root_dir で指定するか、スクリプト内のデフォルト値を変更してください。", file=sys.stderr)
         sys.exit(1) # エラー終了
