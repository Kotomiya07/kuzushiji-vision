"""
YOLOフォーマットのデータセットを準備するスクリプト
1ページ画像と短冊アノテーションを用いてYOLOフォーマットのデータセットを準備する。
書籍単位で分割し、画像は data/raw/dataset からコピーし、アノテーションは column_info_file から取得する。
出力ディレクトリは、train, val, test, no_use の4つのサブディレクトリを持ち、それぞれに images と labels のサブディレクトリが作成される。
