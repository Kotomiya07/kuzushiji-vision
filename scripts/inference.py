...
class KuzushijiRecognizer:
    ...
    def __init__(
        ...
        # 列検出モデルの準備
        with open(column_model_config) as f:
            column_config = EasyDict(yaml.safe_load(f))  # EasyDictでラップ
        ...
        # 文字検出モデルの準備
        with open(char_model_config) as f:
            char_config = EasyDict(yaml.safe_load(f))  # EasyDictでラップ
    ...
    @torch.no_grad()
    def process_image(self, image: Image.Image, visualize: bool = False) -> tuple[list[dict], Image.Image]:
        """画像を処理して文字を検出
        ...
        # 列の検出
        column_image = image.resize((self.column_config.model.input_size[0],) * 2, Image.LANCZOS)
        ...
        results = []
        for column_box, column_score in zip(column_boxes, column_scores, strict=False):
            # 列領域の切り出し
            x1, y1, x2, y2 = map(int, column_box.tolist())
            column_image = image.crop((x1, y1, x2, y2))

            # 文字の検出
            char_image = column_image.resize((self.char_config.model.input_size[0], None), Image.LANCZOS)
        ...
        # 可視化
        if visualize:
            vis_image = visualize_detections(image, results, self.char_config.model.label_map)
...
def main():
    ...
    for image_path in tqdm(image_paths, desc="Processing images"):
        ...
        results, vis_image = recognizer.process_image(image, visualize=config["visualization"]["enabled"])
