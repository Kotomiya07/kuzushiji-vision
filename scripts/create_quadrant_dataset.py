#!/usr/bin/env python3
"""
元画像+4分割データセット作成スクリプト

このスクリプトは既存のYOLOデータセットから元画像と4分割画像の計5枚を生成してデータ拡張を行います。
- 元画像をそのまま保持
- 元画像を2x2に分割して4枚生成
- 対応するアノテーションも適切に変換
- YOLO形式を維持
- 1つの元画像から5枚の画像を生成（5倍のデータ拡張）
...
from src.utils.quadrant_processor import QuadrantProcessor
...
quadrant_dataset:
  ...
  split_method: original_plus_2x2_quadrant
  images_per_original: 5  # 元画像1枚 + 4分割4枚 = 計5枚
  positions:
    - original
    - top_left
    - top_right
        ...
        try:
            ...
            for pos in positions:
                ...
                if split_image_path.exists():
                    ...
                    split_images[pos] = (split_image_rgb, annotations)

            # 可視化プロット作成（3行2列のレイアウト）
            fig, axes = plt.subplots(3, 2, figsize=(12, 18))
            fig.suptitle(f"元画像+4分割結果例 {i + 1}: {image_path.name}", fontsize=16)

            # 元画像の表示（1行目中央に配置）
            axes[0, 0].imshow(original_image_rgb)
            axes[0, 0].set_title(f"元画像 ({original_annotation_count}個)", fontsize=12)
            axes[0, 0].axis("off")
            ...
            for row, col, pos, title in plot_positions:
                if pos in split_images:
                    image, annotations = split_images[pos]
...
def print_summary(stats: dict):
    ...
    for split_name, split_stats in stats.items():
        ...
        generated = processed * 5  # 元画像+4分割なので5倍
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="YOLOデータセットの元画像+4分割によるデータ拡張（5倍化）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  python scripts/create_quadrant_dataset.py
  python scripts/create_quadrant_dataset.py --visualize --sample_count 5
  python scripts/create_quadrant_dataset.py --output_dir data/custom_quadrant_dataset
        """,
    )
    ...
    print("元画像+4分割データセット作成を開始します...")
    print(f"入力ディレクトリ: {args.input_dir}")
    print(f"出力ディレクトリ: {args.output_dir}")
    print(f"重複判定閾値: {args.overlap_threshold}")
    print("※ 1つの元画像から5枚の画像（元画像1枚+4分割4枚）を生成します")

    # プロセッサー初期化
    processor = QuadrantProcessor(
    ...
    try:
        ...
        print("\n元画像+4分割データセット作成が正常に完了しました！")
        print(f"出力先: {args.output_dir}")
        print("各元画像から5枚の画像（元画像1枚+4分割4枚）が生成されました。")
