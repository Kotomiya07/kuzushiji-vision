...
class OCRDataset(Dataset):
    ...
    def _prepare_samples(self):
        ...
        for image_filename in os.listdir(image_dir):
            ...
            if os.path.exists(label_path) and os.path.exists(bbox_path):
                ...
                print(f"Warning: Skipping image {image_filename} due to missing label or bbox file.")
    ...
    def __getitem__(self, idx):
        ...
        # Load label
        with open(sample_info["label_path"], "r", encoding="utf-8") as f:
        ...
        encoded_label = [self.char_to_int.get(char, self.char_to_int["<UNK>"]) for char in label_text[:actual_label_len]] # Use capped length
        ...
        # Load bounding boxes
        with open(sample_info["bbox_path"], "r") as f:
...
def get_data_loader(data_dir, split, batch_size, num_workers, char_to_int, 
                    max_label_length=MAX_LABEL_LENGTH, shuffle=None, 
                    target_height=DEFAULT_TARGET_HEIGHT, target_width=DEFAULT_TARGET_WIDTH):
    """
...
if __name__ == '__main__':
    ...
    if not char_to_int_map or len(char_to_int_map) <= 2 : # Only <PAD> and <UNK>
        print("Warning: char_to_int_map is empty or only contains special tokens. " \
    ...
    try:
        ...
        print("Please ensure that the EXAMPLE_DATA_DIR path is correct and contains the expected subdirectories " \
