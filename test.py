...
def find_images_and_ratios_recursive(directory, high_thresh, low_thresh, extensions):
    ...
    return extreme_images, all_aspect_ratios
...
def plot_aspect_ratio_distribution_below_mean(all_aspect_ratios, bins=50, use_log_scale=False, plot_range=None):
    ...
    plt.axvline(median_filtered, color='green', linestyle='dashed', linewidth=1, label=f'中央値 (表示データ): {median_filtered:.2f}')

    # 全体の平均値も参考線として引く (フィルタリング境界を示す)
    if overall_mean_ratio >= plt.xlim()[0] and overall_mean_ratio <= plt.xlim()[1]: # グラフ範囲内なら表示
        plt.axvline(overall_mean_ratio, color='purple', linestyle='dotted', linewidth=1.5, label=f'全データ平均: {overall_mean_ratio:.2f}')
...
if __name__ == "__main__":
    extreme_image_paths, all_ratios = find_images_and_ratios_recursive(
        TARGET_DIR,
        HIGH_THRESHOLD,
        LOW_THRESHOLD,
